...
require_relative 'UserProjectIntegrator'
...
module Expo
  class AutolinkingManager
    ...
    public def use_expo_modules!
      ...
      Pod::UI.section 'Using Expo modules' do
        @packages.each { |package|
          ...
          if @target_definition.dependencies.any? { |dependency| dependency.name == package.pod_name }
            Pod::UI.puts "— #{Colors.GREEN}#{package.name}#{Colors.RESET} #{Colors.YELLOW}is already added to the target#{Colors.RESET}"
            next
          end
          ...
          Pod::UI.message "— #{package.name.green} (#{package.version})"
        }
      end
      self
    end
